...
class DocumentationFeature:
    ...
    def run(self, user_input: str, *, context: FeatureContext | None = None) -> FeatureResult:
        ...
        try:
            report_type = "BA Report"
            if "SRS" in user_input.upper() or "SPECIFICATION" in user_input.upper():
                report_type = "SRS"

            report_path = BAReportGenerator().generate(ctx.session, report_type=report_type)
            export_note = f"{report_type} exported to {report_path}" if report_path else None
        except Exception as exc:  # pragma: no cover - best effort export
            export_note = f"Report export failed: {exc}"
        if export_note:
